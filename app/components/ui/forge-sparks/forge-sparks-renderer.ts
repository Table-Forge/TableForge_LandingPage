...
type SparkKind = "spark" | "ember" | "mote";
type Range = readonly [number, number];
type Rgb = readonly [number, number, number];
type GlowStop = readonly [offset: number, alpha: number, tone: "core" | "halo"];
...
export interface IForgeSparksRenderer {
...
const FLICKER_RATE: Range = [12, 40];
const FLICKER_HARMONIC = 2.3;
const BURST_INTERVAL: Range = [0.5, 2.4];
const BURST_COUNT: Range = [4, 10];
const BURST_SPEED: Range = [220, 460];
...
const PRESETS: Record<SparkKind, ISparkPreset> = {
  spark: {
    speed: [180, 380],
    spread: 0.3,
    heat: [0.85, 1],
...
const HEAT_RAMP = [FORGE_COLORS.crimson, FORGE_COLORS.orange, FORGE_COLORS.gold, FORGE_COLORS.hot].map(toRgb);
const HEARTH_CORE = toRgb(FORGE_COLORS.orange);
const HEARTH_EDGE = toRgb(FORGE_COLORS.crimson);

const clamp01 = (value: number) => Math.min(Math.max(value, 0), 1);
const randomIn = ([min, max]: Range) => min + Math.random() * (max - min);
...
const heatToRgb = (heat: number): Rgb => {
  const position = clamp01(heat) * (HEAT_RAMP.length - 1);
  const index = Math.min(Math.floor(position), HEAT_RAMP.length - 2);
  const mix = position - index;
  const [fromRed, fromGreen, fromBlue] = HEAT_RAMP[index];
  const [toRed, toGreen, toBlue] = HEAT_RAMP[index + 1];
  return [
...
const createGlowSprite = (color: Rgb, core: Rgb, stops: GlowStop[]) => {
...
const createGlowSprites = (stops: GlowStop[], coreBoost: number) =>
  Array.from({ length: SPRITE_STEPS }, (_, step) => {
    const heat = step / (SPRITE_STEPS - 1);
    return createGlowSprite(heatToRgb(heat), heatToRgb(heat + coreBoost), stops);
  });

export function createForgeSparksRenderer(canvas: HTMLCanvasElement): IForgeSparksRenderer | null {
  const context = canvas.getContext("2d");
  if (!context) return null;

  const sharpSprites = createGlowSprites(SHARP_GLOW, CORE_HEAT_BOOST);
  const softSprites = createGlowSprites(SOFT_GLOW, 0);
  ...
  const launch = (kind: SparkKind, x: number, angle: number, speed: number, heat: number) => {
  ...
  const drawHearth = () => {
    ...
    gradient.addColorStop(0, rgba(HEARTH_CORE, clamp01(intensity)));
    gradient.addColorStop(0.3, rgba(HEARTH_EDGE, clamp01(intensity * 0.6)));
    gradient.addColorStop(1, rgba(HEARTH_EDGE, 0));
